...
#####################################################################################
# Reflat subtitle downloader for python3
# 사용자 설정값
#####################################################################################
# source - 자막없는 영상들이 들어있는 경로: 하위폴더까지검색, 
# dest - 자막을 찾은 경우 이동할 경로
target_list = [
        {'source':'/data/movie/sub_x', 'dest':'/data/movie/sub_o'},
        ]
...
def get_sub_list(keyword):
    ...
    qstr = py_urllib.quote(keyword.encode('utf-8'))
...
def download_sub(sub_seq, video_path):
    ...
    for sub in subs:
        ...
        found = True
        surl = down_url.format(owner=owner, floc=floc, title=py_urllib.quote(title.encode('utf-8')))
    ...
    text = r.text
    if ext == '.smi': lang = detect(get_texts(text))
    else: lang = detect(text)
    cd = chardet.detect(bytes(text, 'utf-8'))
        ...
        for target in target_list:
            ...
            for f in vlist:
                ...
                for rule in search_rules:
                    try:
                        klist = [str(git[key]) for key in rule]
                    except KeyError:
                        continue
